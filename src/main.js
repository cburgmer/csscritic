csscriticLib.main = function (
    ...
    module.add = function (testCase) {
        var augmentedTestCase = util.clone(
            supportUrlAsOnlyTestCaseInput(testCase)
        );

        if (
            currentComponentLabel &&
            augmentedTestCase.component === undefined
        ) {
    ...
    var selectComparisons = function (configuredComparisons) {
        return configuredComparisons.map(function (configuredComparison) {
            return {
                ...
                selected:
                    !selectionFilter ||
                    selectionFilter.isComparisonSelected(configuredComparison),
            };
        });
    };
        ...
        return util.all(
            comparisonSelection.map(function (selection) {
                ...
                return reporting.doReportConfiguredComparison(
                    selection.configuredComparison,
                    isSelected
                );
            })
        );
    ...
    var executeTestCase = function (startingComparison) {
        return regression
            .compare(startingComparison)
            .then(function (comparison) {
                return reporting
                    .doReportComparison(comparison)
                    .then(function () {
                        return comparison;
                    });
            });
    ...
    module.execute = function () {
        ...
        return fetchStartingComparisons(testCases)
            ...
            .then(function () {
                var selectedComparisons = comparisonSelection
